...
fun getGrunnbeløpForMåned(måned: YearMonth): BigDecimal{

    val sisteMåned = findLastChange(måned)

    return grunnbeløp.filter { it.fraMåned.year == sisteMåned.year && it.fraMåned.month == sisteMåned.month }.first().verdi
}
